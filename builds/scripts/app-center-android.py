...
# access_token created from AppCenter Dashboard for only Android app FullAccess 
...
platform_config = config['android']
release_note = 'Android Build Uploaded using python script by ' + platform.node() + ' machine'
...
# clear email txt file first as we run Android build first then append the Android link 
...
with open("./../email.txt", "a") as file:
    file.write(f'\n\nAndroid : {release_url}\n')
    file.write(f'Android : https://install.appcenter.ms/users/{platform_config["owner_name"]}/apps/{platform_config["app_name"]}/releases/{release.identifier}\n')
